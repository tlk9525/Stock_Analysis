...
- Model health: WEAK - Có ngưỡng dương trong sensitivity (chưa phải rule live), nhưng model health chưa sạch: Kiểm thử chiến lược có lợi thế ròng, Độ chính xác cân bằng của mô hình, XGBoost vượt mô hình Logistic đối chứng.
- Nếu chưa có cổ phiếu: WAIT - 0 lệnh mới: xác suất hiện tại 52.6% chưa đạt ngưỡng theo dõi 0.62.
...
|---|---:|---|
| Lệnh mới hôm nay | 0 lệnh | 0 lệnh khi chưa ACTIONABLE. Không chia nhỏ/DCA để che một backtest có turnover cao. |
| Model health | WEAK | Có ngưỡng dương trong sensitivity (chưa phải rule live), nhưng model health chưa sạch: Kiểm thử chiến lược có lợi thế ròng, Độ chính xác cân bằng của mô hình, XGBoost vượt mô hình Logistic đối chứng. |
| Nếu chưa có cổ phiếu | WAIT | Không có lệnh mua mới hôm nay |
| Lý do cho mua mới |  | 0 lệnh mới: xác suất hiện tại 52.6% chưa đạt ngưỡng theo dõi 0.62. |
| Nếu đang nắm giữ | HOLD_DISCIPLINED | Chưa đủ điều kiện mua mới, nhưng kỹ thuật chưa xấu; nếu đang giữ thì có thể giữ có stop-loss, không mua thêm. |
| Xác suất hiện tại | 52.6% | So với ngưỡng sau phí được chọn từ OOS. |
| Ngưỡng theo dõi từ sensitivity | 0.62 | Ngưỡng 0.62; net +6.8%; 20 vòng; chưa phải rule live. |
...
- Điều kiện phát hành tín hiệu: Swing strategy chưa qua frozen holdout + stress phí 1.5x.
- Điều kiện phát hành tín hiệu: Expected excess return swing chưa vượt chi phí + safety margin: expected=0.0006463585887104273, required>0.0100.
